...
def commit(objeto, fileName):
    file = open("data/isam/" + fileName + ".bin", "wb+")
...
def rollback(fileName):
    file = open("data/isam/" + fileName + ".bin", "rb")
    b = file.read()
    file.close()
...
def checkDirs():
    if not os.path.exists('data'):
        os.makedirs('data')
    if not os.path.exists('data/isam'):
        os.makedirs('data/isam')
    if not os.path.exists('data/isam/databasesISAM.bin'):
        databases = []
        commit(databases, 'databasesISAM')
    if not os.path.exists('data/isam/tables'):
        os.makedirs('data/isam/tables')
