...
def checkData():
    if not os.path.isdir("data"):
        os.mkdir("data")
    if not os.path.isdir("data/bplus"):
        os.mkdir("data/bplus")
    if not os.path.isfile("data/bplus/Databases.bin"):
        with open("data/bplus/Databases.bin", 'wb') as f:
...
def createDatabase(database):
    ...
    if database and validateIdentifier(database):
        dataBaseTree = serializable.Read('./data/bplus/', 'Databases')
        root = dataBaseTree.getRoot()
        if dataBaseTree.search(root, database.upper()):
            return 2
        else:
            dataBaseTree.add(root, database.upper())
            serializable.write('./data/bplus/', database, AVLTree.AVLTree())
            serializable.update('./data/bplus/', 'Databases', dataBaseTree)
        return 0
    else:
        return 1
...
def alterDatabase(dataBaseOld, dataBaseNew) -> int:
    ...
    if validateIdentifier(dataBaseOld) and validateIdentifier(dataBaseNew):
        ...
        root = dataBaseTree.getRoot()
        serializable.Rename('./data/bplus/', dataBaseOld, dataBaseNew)
...
def dropDatabase(database):
    ...
    if validateIdentifier(database):
        ...
        serializable.delete('./data/bplus/' + database)
        serializable.update('./data/bplus/', 'Databases', dataBaseTree)
        return 0
    else:
        return 1
...
def createTable(database, table, numberColumns):
    ...
    if validateIdentifier(database) and validateIdentifier(table) and numberColumns > 0:
        ...
        # If DB exist
        if databaseNode:
            tablesTree = serializable.Read(f"./data/bplus/{database}/", database)
            if tablesTree.search(tablesTree.getRoot(), table.upper()):
                return 3
            else:
                # Creates new table node
                tablesTree.add(tablesTree.getRoot(), table.upper())
                serializable.update(f"./data/bplus/{database}/", database, tablesTree)
                # Creates bin file for the new table
                serializable.write(f"./data/bplus/{database}/", table, BplusTree.BPlusTree(5, numberColumns))
                return 0
        else:
            return 2
    else:
...
def showTables(database):
    ...
    if validateIdentifier(database):
        ...
        if dataBaseTree.search(dataBaseTree.getRoot(), database.upper()):
            db = serializable.Read(f"./data/bplus/{database}/", database)
...
def extractTable(database, table):
    ...
    if validateIdentifier(database) and validateIdentifier(table):
        ...
        if databaseNode:
            tablesTree = serializable.Read(f"./data/bplus/{database}/", database)
            if tablesTree.search(tablesTree.getRoot(), table.upper()):
                table = serializable.Read(f'./data/bplus/{database}/{table}/', table)
        ...
        if validateIdentifier(database) and validateIdentifier(table):
            ...
            if databaseNode:
                ...
                if not tablesTree.search(tablesTree.getRoot(), table.upper()):
                    ...
                    tuplaTree = serializable.Read(f"./data/bplus/{database}/{table}/", table)
                    try:
                    ...
                    if res:
                        ...
                        serializable.update(f"./data/bplus/{database}/{table}/", table, tuplaTree)
                        return 0
            else:
    ...
    try:
        ...
        if validateIdentifier(database) and validateIdentifier(table):
            ...
            if not dataBaseTree.search(root, database.upper()):
                ...
                if not tablesTree.search(tablesTree.getRoot(), table.upper()):
                    return 3  # table no existente
                PKsTree = serializable.Read(f'./data/bplus/{database}/{table}/', table)
                res = PKsTree.DeletePk()
                if res:
                    return res
                else:
                    serializable.update(f'./data/bplus/{database}/{table}/', table, PKsTree)
                return 0  # exito
        else:
            return 1
    except:
...
def alterTable(database: str, tableOld: str, tableNew: str) -> int:
    ...
    if validateIdentifier(tableOld) and validateIdentifier(tableNew):
        ...
        if databaseNode:
                ...
                serializable.Rename(f'./data/bplus/{database}/', tableOld, tableNew)
    ...
    try:
        ...
        if databaseNode:
            ...
            if not tablesTree.search(root, table.upper()):
                ...
                serializable.delete(f"./data/bplus/{database}/{table}")
