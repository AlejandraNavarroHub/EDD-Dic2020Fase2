...
class Databases_Window:
    ...
    def Settings_Frame(self, database):
        ...
        Label(self.new_frame(temp, 0, 0, 20, 40), text="No. Tables: ", font=("Arial Black", 8)).pack()
        Label(self.new_frame(temp, 0, 0, 105, 40), text=str(len(h.showTables(database))), font=("Arial", 9)).pack()

        Label(self.new_frame(temp, 0, 0, 140, 40), text="Mode: ", font=("Arial Black", 8)).pack()
        Label(self.new_frame(temp, 0, 0, 190, 40), text=str(h._database(database)["modo"]), font=("Arial", 9)).pack()
    ...
    def _command_create_database(self):
        ...
        Label(self.new_frame(temp, 0, 0, 10, 75), text="Mode: ", font=("Arial Black", 8)).pack()
        combomode= ttk.Combobox(self.new_frame(temp, 40, 10, 70, 75), width=15, state="readonly")
        combomode["values"] = ["avl", "b", "bplus", "hash", "isam", "json", "dict"]
        combomode.pack()

        Label(self.new_frame(temp, 0, 0, 10, 105), text="Encording: ", font=("Arial Black", 8)).pack()
        comboencording = ttk.Combobox(self.new_frame(temp, 40, 10, 100, 105), width=15, state="readonly")
        comboencording["values"] = ["utf8", "ascii", "iso-8859-1"]
        comboencording.pack()

        Button(self.new_frame(temp, 0, 0, 80, 150), text="Aceptar", font=("Arial", 12)
               , command=self.Create_Database(temp, Entrada, combomode, comboencording)).pack()
        Button(self.new_frame(temp, 0, 0, 170, 150), text="Cancelar", font=("Arial", 12),
               command=self.Cancel(temp, "")).pack()

        x = (temp.winfo_screenwidth() // 2) - (temp.winfo_width() // 2) - 200
        y = (temp.winfo_screenheight() // 2) - (temp.winfo_height() // 2) - 100
        temp.geometry('{}x{}+{}+{}'.format(350, 200, x, y))
    ...
    def Create_Database(self, tmp, name, mode, encording):
        return lambda : self._Create_Database(tmp, name, mode, encording)

    def _Create_Database(self,tmp, name, mode, encording):
        print(mode.get(), encording.get())
        if str(name.get()) != "":
            temp = h._database(name.get())
            if temp:
                text = "This database already exists"
                tmp.destroy()
                self.Warning_Window_Create_Database(text)
            else:                
                print(h.createDatabase(name.get(),mode.get(),encording.get()))
                tmp.destroy()
...
class Tuples_Window:
    def __init__(self, win,  database, table, settings_frame):
        ...
        Label(self.window, text="DATABASE: "+str(database), font=("Arial Black", 8)).place(x=70, y=10)
        Label(self.window, text="TABLE: " + str(table), font=("Arial Black", 8)).place(x=70, y=35)
        Label(self.window, text="TUPLE | PRIMARY KEY", font=("Arial Black", 8)).place(x=20, y=125)
        ...
        Label(self.window, text="INSERT A NEW TUPLE ", font=("Arial Black", 8)).place(x=140, y=80)
    ...
    def show_tuples(self, frame, database, table):
        ...
        if tmp["pk"]:
            for j in h.extractTable(database, table):
                Button(frame, text="• " + str(j[int(tmp["pk"][0])]), width=30, anchor="w",
                       command=self._show_tuples(database, table, str(j[int(tmp["pk"][0])]))).pack()
        else:
            for j in h.extractTable(database, table):
                Button(frame, text="• " + str(j[0]), width=30, anchor="w",
                       command=self._show_tuples(database, table, "")).pack()
        pass
    ...
    def _command_extract_row(self, database, table, pk):
        ...
        Label(self.new_frame(temp, 100, 30, 150, 10), text=str(pk), font=("Arial", 11)).pack()
        Frame1 = LabelFrame(temp)
        can = Canvas(Frame1, width=430, height=20)
        Scrollbar_x = Scrollbar(Frame1, orient="vertical", command=can.yview)
        Scrollbar_y = Scrollbar(Frame1, orient="horizontal", command=can.xview)
        ...
        can.bind("<Configure>", lambda e: can.configure(scrollregion=can.bbox("all")))
        Frame1.place(x=20, y=50)
        Label(frame, text=str(h.extractRow(database,table,[pk])), font=("Arial", 10)).pack()
        x = (temp.winfo_screenwidth() // 2) - (temp.winfo_width() // 2) - 200
        y = (temp.winfo_screenheight() // 2) - (temp.winfo_height() // 2) - 100
        temp.geometry('{}x{}+{}+{}'.format(500, 140, x, y))
        temp.resizable(0, 0)
    ...
    def _command_update(self,database, table, pk):
        ...
        temp = Toplevel()
        Label(self.new_frame(temp, 0, 0, 10, 10), text="TABLE: " + str(table), font=("Arial Black", 8)).pack()
        #Label(self.new_frame(temp, 0, 0, 230, 10), text="COLUMNS: " + str(tmp["numero_columnas"]), font=("Arial Black", 8)).pack()
    ...
    def _update_tuple(self, tmp, database, table, pk, entrada):
        ...
        try:
            ...
            print(str(list))
            print(h.update(database, table, list, [pk]))
            tmp.destroy()
            Tuples_Window(self.window, self.database, self.table, self.Default_Frame())
        except:
            tmp.destroy()
    ...
    def _command_delete(self, database, table, pk):
        ...
        Label(self.new_frame(temp, 0, 0, 140, 43), text=str(pk), font=("Arial", 9)).pack()
        Button(self.new_frame(temp, 0, 0, 80, 80), text="Aceptar", font=("Arial", 12),
    ...
    def _delete_tuple(self, tmp, database, table, pk):
        print(h.delete(database, table, [pk]))
        tmp.destroy()
        Tuples_Window(self.window, self.database, self.table, self.Default_Frame())
